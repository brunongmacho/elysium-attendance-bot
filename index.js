...
if (message.content.toLowerCase() === '!testbidding') {
  ...
  // Optional: acknowledge in the channel where command was typed
  message.reply('🔍 Running system diagnostics... please wait.').catch(console.error);
  ...
  fetch(config.sheet_webhook_url, {
    ...
    .finally(() => {
      ...
      client.channels
        .fetch(config.admin_logs_channel_id) // ✅ POST TO ADMIN LOGS CHANNEL
        .then((adminLogChannel) => {
          if (adminLogChannel) {
            channelTest.canAccessChannel = true;
            channelTest.channelName = adminLogChannel.name;
            channelTest.isThread = adminLogChannel.isThread();
              ...
              .addFields(
                ...
                },
                {
                  ...
                  value: channelTest.canAccessChannel
                    ...
                    : `❌ **Cannot access admin log channel**\n` +
            ...
            adminLogChannel.send({ embeds: [embed] }).catch(console.error);
          }
        })
        .catch((err) => {
          console.error('❌ Failed to access admin logs channel:', err);
        });
  ...
  return;
    ...
    if (inBiddingChannel) {
      ...
      console.log(
        `🎯 Bidding command detected: ${command} in channel ${message.channel.id}`
      );
