package com.tenchu.guild.viewmodel
...
import com.tenchu.guild.utils.ErrorUtils
...
class BossTimersViewModel @Inject constructor(
    ...
    fun toggleTenchuTurnFilter() {
        viewModelScope.launch {
            val next = !_uiState.value.onlyTenchuTurn
            val filtered = withContext(Dispatchers.Default) {
                applyFilter(
                    _uiState.value.bosses,
                    _uiState.value.selectedFilter,
...
data class BossTimersUiState(
    ...
    val onlyTenchuTurn: Boolean = false,
